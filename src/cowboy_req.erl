...
-spec host_info(Req) -> {cowboy_dispatcher:tokens() | undefined, Req}
	when Req::req().
host_info(Req) ->
...
-spec path_info(Req) -> {cowboy_dispatcher:tokens() | undefined, Req}
	when Req::req().
path_info(Req) ->
...
-spec qs_val(binary(), Req) -> {binary() | true | undefined, Req}
	when Req::req().
...
%% missing.
-spec qs_val(binary(), Req, Default) -> {binary() | true | Default, Req}
	when Req::req(), Default::any().
...
-spec binding(atom(), Req, Default) -> {binary() | Default, Req}
	when Req::req(), Default::any().
...
-spec header(atom() | binary(), Req) -> {binary() | undefined, Req}
	when Req::req().
...
-spec header(atom() | binary(), Req, Default) -> {binary() | Default, Req}
	when Req::req(), Default::any().
...
-spec parse_header(cowboy_http:header(), Req)
	-> {any(), Req} | {error, badarg} when Req::req().
...
parse_header_default(_) -> undefined.
...
-spec cookie(binary(), Req) -> {binary() | true | undefined, Req}
	when Req::req().
...
%% missing.
-spec cookie(binary(), Req, Default) -> {binary() | true | Default, Req}
	when Req::req(), Default::any().
...
-spec stream_body(Req) -> {ok, binary(), Req} | {done, Req} | {error, atom()}
	when Req::req().
...
-spec stream_body_recv(Req) -> {ok, binary(), Req} | {error, atom()}
	when Req::req().
...
-spec transfer_decode(binary(), Req) -> {ok, binary(), Req} | {error, atom()}
	when Req::req().
...
-spec transfer_decode_done(non_neg_integer(), binary(), Req) -> Req
	when Req::req().
...
-spec multipart_data(Req)
	-> {{headers, cowboy_http:headers()} | {body, binary()} | end_of_part | eof,
		Req} when Req::req().
...
-spec set_resp_header(cowboy_http:header(), iodata(), Req) -> {ok, Req}
	when Req::req().
